...
describe Apia::LookupArgumentSet do
  context '#validate' do
    ...
    subject(:argument) do
      a = Apia::Definitions::Argument.new(:user)
      a.type = lookup_as
      a
    end

    it 'should raise an error if no options are provide3d' do
      expect { lookup_as.new({}).validate(argument) }.to raise_error(Apia::InvalidArgumentError) do |e|
        expect(e.issue).to eq :missing_lookup_value
      end
    end

    it 'should raise an error if more than one option is provided' do
      expect { lookup_as.new({ id: 1234, permalink: 'some-permalink' }).validate(argument) }.to raise_error(Apia::InvalidArgumentError) do |e|
  ...
  context '#resolve' do
    ...
    it 'should be able to raise errors' do
      ...
      expect { klass.new({ id: 1 }).resolve }.to raise_error Apia::ErrorExceptionError do |e|
