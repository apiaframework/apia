...
describe Apia::Definitions::Endpoint do
  context '#argument_set' do
    it 'should provide a default argument set for the endpoint' do
      as = Apia::Definitions::Endpoint.new('ExampleEndpoint')
      expect(as.argument_set.ancestors).to include Apia::ArgumentSet
  ...
  context '#validate' do
    ...
    it 'should add errors if any potential error is not an appropriate class' do
      endpoint = described_class.new('Endpint')
      endpoint.potential_errors << Apia::Controller.create('MyController')
      errors = Apia::ManifestErrors.new
      endpoint.validate(errors)
    ...
    it 'should add errors if the authenticator is not an authenticator' do
      endpoint = described_class.new('Endpint')
      endpoint.authenticator = Apia::Controller.create('MyController')
      errors = Apia::ManifestErrors.new
      endpoint.validate(errors)
